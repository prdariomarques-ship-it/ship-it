...
- **Handshake de verificação do webhook da WhatsApp Cloud API.** `GET /webhooks/whatsapp` implementado — a Meta exige essa verificação (`hub.mode=subscribe` + `hub.verify_token` + `hub.challenge`) antes de aceitar salvar a URL do webhook no App Dashboard; sem ela, a migração para o provider `official` não tem como ser configurada do lado da Meta. Token comparado com `OFFICIAL_WEBHOOK_VERIFY_TOKEN` (novo, `.env`) via `hmac.compare_digest`; responde `hub.challenge` em texto puro quando válido, `403` caso contrário (token ausente/errado ou `hub.mode` diferente de `subscribe`). `official_api_base_url` também atualizado de `v21.0` para `v22.0` da Graph API. 4 testes novos em `test_webhook_security.py`.
...
Cinco achados de prioridade média da homologação funcional (`HOMOLOGATION_REPORT.md`, 2026-07-18) corrigidos:

- **Unificar a fonte de versão.** `app_version` era um literal hardcoded (`0.2.1`) em `utils/config.py`, dessincronizado do `VERSION.json` real (que também estava desatualizado, `1.3.0-rc1`) — `/health` e `/admin/system` mostravam uma versão errada pro usuário. Extraída a leitura de `VERSION.json` para `utils/version_file.py`, compartilhada por `config.py` e pelo endpoint `/api/version` (que tinha uma implementação duplicada, agora removida); `VERSION.json` regenerado com a versão/commit reais.
- **Corrigir estatísticas de memória do Qdrant.** `/admin/memory` mostrava "Pontos no Qdrant: 88" e "Vetores: 0" para a mesma coleção — `vectors_count` lia `indexed_vectors_count` (vetores promovidos ao índice HNSW, fica em 0 abaixo do `indexing_threshold` mesmo com dados reais) em vez de `points_count`. Corrigido para usar `points_count`, já que este schema é um vetor por ponto.
- **Localizar labels do Analytics.** `/analytics` derivava labels das chaves em inglês da API (`CONTACTS`, `PENDING TASKS`...) para os mesmos dados que a Início mostra em português (`Contatos`, `Tarefas pendentes`...). Agora usa os mesmos labels.
- **Melhorar navegação de Configurações.** `/configuracoes` prometia "preferências e conexões" que não existiam ali (só endpoint da API e botão Sair). Texto corrigido para descrever o conteúdo real, com links para onde Google Workspace/WhatsApp são de fato geridos (`/admin/google`, `/admin/whatsapp`, `/admin/settings`).
- **Consolidar a página de Logs.** `/logs` e `/admin/logs` serviam os mesmos dados admin-only (`LogEntry`), mas só a segunda tinha filtro por nível, busca e exportar — a primeira mostrava só ruído de `job:observation.tick` sem filtro nenhum. `/logs` agora redireciona (`307`) para `/admin/logs`; link da sidebar atualizado.

### Testes

Cobertura nova em `test_providers.py` (success=true, success=false nos dois formatos de erro do easy-api, campo `success` ausente, JSON malformado, HTTP 500, e os 4 métodos de mídia/localização individualmente) e um teste de integração em `test_audit_fixes.py` confirmando que a rota `/api/whatsapp/send-text` responde `502` — nunca `{"status": "sent"}` — e não persiste `Message` quando o envio é rejeitado. Mais um teste de regressão em `test_admin.py` (vectors_count == points_count) e dois testes existentes em `test_version.py` atualizados (esperavam o valor antigo de versão como correto). Suite completa: 875 testes de backend, 240 de frontend, 100% passando.
