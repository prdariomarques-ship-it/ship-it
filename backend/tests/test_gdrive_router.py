...
async def test_connect_forbidden_for_non_admin(client, second_user_headers):
    response = await client.get("/api/gdrive/connect", headers=second_user_headers)
...
async def test_connect_unconfigured_returns_503(client, auth_headers, monkeypatch):
    monkeypatch.setattr(get_settings(), "google_drive_redirect_uri", "")
    response = await client.get("/api/gdrive/connect", headers=auth_headers)
...
async def test_callback_missing_code_or_state_fails_cleanly(client):
    response = await client.get("/api/gdrive/oauth/callback")
    assert response.status_code == 200
...
async def test_callback_escapes_the_error_param_against_reflected_xss(client):
    ...
    response = await client.get("/api/gdrive/oauth/callback", params={"error": payload})
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_callback_rejects_a_gcalendar_state_token(client):
    """Cross-domain isolation: a state token minted for the Calendar
    callback must not be accepted by the Drive callback."""
    calendar_state = create_oauth_state_token(1, purpose="gcalendar_oauth_state")
    response = await client.get(
        "/api/gdrive/oauth/callback", params={"code": "abc", "state": calendar_state}
    )
    assert response.status_code == 200
...
async def test_callback_rejects_a_state_token_of_the_wrong_purpose(client):
    ...
    response = await client.get(
        "/api/gdrive/oauth/callback", params={"code": "abc", "state": not_a_state_token}
    )
    assert response.status_code == 200
    assert "Falha" in response.text or "inválida" in response.text
...
async def test_callback_success_stores_an_encrypted_refresh_token(client, auth_headers, db_engine):
    ...
    async with factory() as session:
        account = (await session.execute(select(GoogleDriveAccount))).scalar_one()
...
async def test_callback_reconnect_updates_the_existing_account_instead_of_duplicating(
    ...
    factory = async_sessionmaker(db_engine, expire_on_commit=False)
    async with factory() as session:
        accounts = (await session.execute(select(GoogleDriveAccount))).scalars().all()
...
async def test_callback_recovers_when_two_concurrent_callbacks_race_on_create(
    ...
    original_get_by_user = GoogleDriveAccountRepository.get_by_user
    calls = {"count": 0}
    ...
    monkeypatch.setattr(GoogleDriveAccountRepository, "get_by_user", racy_get_by_user)
    ...
    factory = async_sessionmaker(db_engine, expire_on_commit=False)
    async with factory() as session:
        rows = (await session.execute(select(GoogleDriveAccount))).scalars().all()
    assert len(rows) == 1
...
async def test_status_forbidden_for_non_admin(client, second_user_headers):
    response = await client.get("/api/gdrive/status", headers=second_user_headers)
    assert response.status_code == 403
...
async def test_disconnect_is_a_no_op_when_nothing_is_connected(client, auth_headers):
    response = await client.delete("/api/gdrive/disconnect", headers=auth_headers)
