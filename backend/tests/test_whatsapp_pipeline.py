...
async def _jobs_named(session_factory, name: str) -> list[Job]:
    async with session_factory() as session:
        jobs = (
            (await session.execute(select(Job).where(Job.name == name))).scalars().all()
        )
        return list(jobs)


@pytest.mark.asyncio
async def test_full_pipeline_webhook_to_reply(
    ...
    assert send_jobs[0].payload[
        "content"
    ]  # the agent's reply text (stub, no LLM key in tests)
...
@pytest.mark.asyncio
async def test_auto_reply_acts_on_behalf_of_first_admin(
    client, auth_headers, session_factory, worker
):
    ...
    assert (
        owner.email == "dario@example.com"
    )  # from the auth_headers fixture, the first registered user
...
@pytest.mark.asyncio
async def test_pipeline_skips_auto_reply_without_any_registered_user(
    client, session_factory, worker
):
    ...
    async with session_factory() as session:
        job = (
            await session.execute(
                select(Job).where(Job.name == "whatsapp.process_inbound")
            )
        ).scalar_one()
...
@pytest.mark.asyncio
async def test_auto_reply_disabled_by_setting(
...
@pytest.mark.asyncio
async def test_loop_guard_throttles_auto_reply_per_contact(
    ...
    assert (
        len(process_jobs) == 2
    )  # only the first 2 within the window triggered a reply
...
async def test_send_text_job_persists_and_feeds_memory(session_factory, worker):
    ...
    async with session_factory() as session:
        ...
        await JobService(session).enqueue(
            "whatsapp.send_text",
            {"to": "5511955556667", "content": "Olá direto da fila!"},
        )
...
@pytest.mark.asyncio
async def test_apology_sent_after_auto_reply_job_exhausts_retries(
