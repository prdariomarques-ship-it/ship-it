...
_PURPOSE = "gcontacts_oauth_state"
...
async def test_connect_forbidden_for_non_admin(client, second_user_headers):
    response = await client.get("/api/gcontacts/connect", headers=second_user_headers)
    assert response.status_code == 403


@pytest.mark.asyncio
async def test_connect_requires_authentication(client):
    response = await client.get("/api/gcontacts/connect")
    assert response.status_code == 401


@pytest.mark.asyncio
async def test_connect_unconfigured_returns_503(client, auth_headers, monkeypatch):
    monkeypatch.setattr(get_settings(), "google_contacts_redirect_uri", "")
    response = await client.get("/api/gcontacts/connect", headers=auth_headers)
...
async def test_callback_missing_code_or_state_fails_cleanly(client):
    response = await client.get("/api/gcontacts/oauth/callback")
    assert response.status_code == 200
...
async def test_callback_rejects_a_gcalendar_state_token(client):
    ...
    callback must not be accepted by the Contacts callback, even though
    both reuse the same signing helper."""
    calendar_state = create_oauth_state_token(1, purpose="gcalendar_oauth_state")
    response = await client.get(
        "/api/gcontacts/oauth/callback", params={"code": "abc", "state": calendar_state}
    )
    assert response.status_code == 200
...
async def test_callback_success_stores_an_encrypted_refresh_token(
    ...
    async with factory() as session:
        account = (await session.execute(select(GoogleContactsAccount))).scalar_one()
...
async def test_callback_recovers_when_two_concurrent_callbacks_race_on_create(
    ...
    original_get_by_user = GoogleContactsAccountRepository.get_by_user
    calls = {"count": 0}
    ...
    async with session_factory() as session:
        second = await GoogleContactsAccountRepository(session).upsert_for_user(
            user_id,
            "google",
            account_label="racer",
    ...
    factory = async_sessionmaker(db_engine, expire_on_commit=False)
    async with factory() as session:
        rows = (await session.execute(select(GoogleContactsAccount))).scalars().all()
    assert len(rows) == 1
...
async def test_status_forbidden_for_non_admin(client, second_user_headers):
    response = await client.get("/api/gcontacts/status", headers=second_user_headers)
    assert response.status_code == 403
...
async def test_disconnect_is_a_no_op_when_nothing_is_connected(client, auth_headers):
    response = await client.delete("/api/gcontacts/disconnect", headers=auth_headers)
