...
# --- /admin (index) -------------------------------------------------------------
@pytest.mark.asyncio
async def test_admin_index_reflects_seeded_data(client, admin_headers, admin_user, regular_user):
...
async def test_admin_status_reports_every_expected_component(client, admin_headers):
    ...
    assert names == {
        "backend", "database", "redis", "qdrant", "whatsapp", "event_bus", "memory", "google_oauth",
    }
...
@pytest.mark.asyncio
async def test_admin_system_exposes_provider_names_but_never_secrets(client, admin_headers):
    response = await client.get("/api/admin/system", headers=admin_headers)
    body = response.json()
...
# --- /admin/agents ----------------------------------------------------------------
@pytest.mark.asyncio
async def test_admin_agents_lists_every_registered_agent_with_tool_counts(client, admin_headers):
    ...
    for item in body:
        ...
        assert item["last_execution"] is None  # no LogEntry seeded for any agent in this test


@pytest.mark.asyncio
async def test_admin_agents_reflects_real_prometheus_counters_end_to_end(client, admin_headers):
    ...
    record_agent_run(agent="personal", provider="openai", status="ok", duration_seconds=2.0)
    record_agent_run(agent="personal", provider="openai", status="ok", duration_seconds=4.0)
...
# --- /admin/tools -----------------------------------------------------------------
@pytest.mark.asyncio
async def test_admin_tools_marks_unavailable_fields_as_null_not_fabricated(client, admin_headers):
...
@pytest.mark.asyncio
async def test_admin_tools_reflects_real_prometheus_counters_end_to_end(client, admin_headers):
...
# --- /admin/logs ------------------------------------------------------------------
@pytest.mark.asyncio
async def test_admin_logs_filters_by_level_and_search(client, admin_headers, session_factory):
    async with session_factory() as session:
        session.add_all(
            [
                LogEntry(level="error", source="agent:assistant", message="boom", payload={}),
                LogEntry(level="info", source="webhook", message="all good", payload={}),
            ]
        )
        await session.commit()

    response = await client.get("/api/admin/logs", headers=admin_headers, params={"level": "error"})
    body = response.json()
    assert len(body) == 1
    assert body[0]["message"] == "boom"

    response = await client.get("/api/admin/logs", headers=admin_headers, params={"search": "good"})
    body = response.json()
    assert len(body) == 1
    assert body[0]["source"] == "webhook"

    response = await client.get("/api/admin/logs", headers=admin_headers, params={"source": "webhook"})
    body = response.json()
    assert len(body) == 1
    assert body[0]["source"] == "webhook"
...
# --- /admin/memory ----------------------------------------------------------------
@pytest.mark.asyncio
async def test_admin_memory_counts_embeddings_by_source(client, admin_headers, session_factory):
    async with session_factory() as session:
        session.add_all(
            [
...
# --- /admin/executions ------------------------------------------------------------
@pytest.mark.asyncio
async def test_admin_executions_merges_jobs_and_agent_logs(client, admin_headers, session_factory):
    async with session_factory() as session:
        session.add(Job(name="whatsapp.process_inbound", status=JobStatus.SUCCEEDED, payload={}))
        session.add(LogEntry(level="info", source="agent:personal", message="ran", payload={}))
        session.add(LogEntry(level="info", source="webhook", message="not an agent log", payload={}))
        await session.commit()

    response = await client.get("/api/admin/executions", headers=admin_headers, params={"period": "24h"})
    body = response.json()
    kinds = {entry["kind"] for entry in body}
    assert kinds == {"job", "log"}
    names = {entry["name"] for entry in body}
    assert "webhook" not in names  # only agent:* logs are surfaced, not every log source


@pytest.mark.asyncio
async def test_admin_executions_filters_by_agent(client, admin_headers, session_factory):
    async with session_factory() as session:
        session.add(Job(name="whatsapp.process_inbound", status=JobStatus.SUCCEEDED, payload={}))
        session.add(LogEntry(level="info", source="agent:personal", message="ran", payload={}))
        session.add(LogEntry(level="info", source="agent:assistant", message="also ran", payload={}))
        await session.commit()

    response = await client.get(
        "/api/admin/executions", headers=admin_headers, params={"period": "24h", "agent": "personal"}
    )
    body = response.json()
...
async def test_admin_executions_rejects_invalid_period(client, admin_headers):
    response = await client.get("/api/admin/executions", headers=admin_headers, params={"period": "invalid"})
    assert response.status_code == 422


# --- /admin/users -----------------------------------------------------------------
@pytest.mark.asyncio
async def test_admin_users_never_leaks_password_hash(client, admin_headers, admin_user, regular_user):
    response = await client.get("/api/admin/users", headers=admin_headers)
    assert "hashed_password" not in response.text
    body = response.json()
    assert {item["email"] for item in body} == {"admin@darioos.test", "user@darioos.test"}
...
# --- /admin/whatsapp --------------------------------------------------------------
@pytest.mark.asyncio
async def test_admin_whatsapp_counts_messages_by_direction(client, admin_headers, session_factory):
    ...
    async with session_factory() as session:
        ...
        session.add_all(
            [
                Message(
                    contact_id=contact.id, direction=MessageDirection.OUTBOUND,
                    media_type=MessageMediaType.TEXT, content="oi",
                ),
                Message(
                    contact_id=contact.id, direction=MessageDirection.INBOUND,
                    media_type=MessageMediaType.TEXT, content="oi de volta",
                ),
                Message(
                    contact_id=contact.id, direction=MessageDirection.INBOUND,
                    media_type=MessageMediaType.TEXT, content="mais uma",
                ),
            ]
        )
        await session.commit()
...
def test_metric_value_accumulates_only_matching_samples():
    ...
    snapshot = {
        "darioos_agent_runs_total": [
            {"name": "darioos_agent_runs_total", "labels": {"agent": "personal", "status": "ok"}, "value": 3.0},
            {"name": "darioos_agent_runs_total", "labels": {"agent": "personal", "status": "error"}, "value": 1.0},
            {"name": "darioos_agent_runs_total", "labels": {"agent": "assistant", "status": "ok"}, "value": 9.0},
        ]
    }
    assert _metric_value(snapshot, "darioos_agent_runs_total", agent="personal", status="ok") == 3.0
...
def test_agent_run_stats_computes_average_duration_from_histogram_sum_and_count():
    ...
    snapshot = {
        "darioos_agent_runs_total": [
            {"name": "darioos_agent_runs_total", "labels": {"agent": "personal", "status": "ok"}, "value": 4.0},
        ],
        "darioos_agent_run_duration_seconds_sum": [
            {"name": "darioos_agent_run_duration_seconds_sum", "labels": {"agent": "personal"}, "value": 8.0},
        ],
        "darioos_agent_run_duration_seconds_count": [
            {"name": "darioos_agent_run_duration_seconds_count", "labels": {"agent": "personal"}, "value": 4.0},
        ],
    }
...
def test_tool_call_stats_counts_ok_and_error_separately():
    ...
    snapshot = {
        "darioos_agent_tool_calls_total": [
            {"name": "darioos_agent_tool_calls_total", "labels": {"tool": "send_whatsapp_message", "status": "ok"}, "value": 5.0},
            {"name": "darioos_agent_tool_calls_total", "labels": {"tool": "send_whatsapp_message", "status": "error"}, "value": 2.0},
        ]
    }
...
@pytest.mark.asyncio
async def test_drive_index_stats_returns_zero_and_none_when_nothing_indexed(session_factory):
