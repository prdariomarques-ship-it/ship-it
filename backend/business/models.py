...
class Client(Base):
    ...
    kpis: Mapped[list["KPI"]] = relationship(
        "KPI", back_populates="client", foreign_keys="KPI.client_id"
    )
...
class Deal(Base):
    ...
    client_id: Mapped[int] = mapped_column(
        BIGINT, ForeignKey("clients.id"), nullable=False
    )
    ...
    followups: Mapped[list["Followup"]] = relationship(
        "Followup", back_populates="deal"
    )
    projects: Mapped[list["Project"]] = relationship("Project", back_populates="deal")
    kpis: Mapped[list["KPI"]] = relationship(
        "KPI", back_populates="deal", foreign_keys="KPI.deal_id"
    )
...
class Project(Base):
    ...
    __table_args__ = (Index("projects_deal_id_idx", "deal_id"),)
    ...
    spent: Mapped[Optional[Decimal]] = mapped_column(
        NUMERIC(precision=12, scale=2), default=Decimal("0"), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        TIMESTAMP(timezone=True), default=datetime.utcnow, nullable=False
    )
...
class KPI(Base):
    ...
    client_id: Mapped[Optional[int]] = mapped_column(
        BIGINT, ForeignKey("clients.id"), nullable=True
    )
    deal_id: Mapped[Optional[int]] = mapped_column(
        BIGINT, ForeignKey("deals.id"), nullable=True
    )
    ...
    client: Mapped[Optional[Client]] = relationship(
        "Client", back_populates="kpis", foreign_keys=[client_id]
    )
    deal: Mapped[Optional[Deal]] = relationship(
        "Deal", back_populates="kpis", foreign_keys=[deal_id]
    )
