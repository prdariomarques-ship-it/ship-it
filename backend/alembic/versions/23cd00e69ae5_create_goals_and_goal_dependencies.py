...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["recurrence_parent_id"], ["goals.id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        op.f("ix_goals_recurrence_parent_id"),
        "goals",
    ...
    op.create_index(
        op.f("ix_goal_dependencies_goal_id"),
        "goal_dependencies",
        ["goal_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_goal_dependencies_depends_on_id"),
        "goal_dependencies",
        ["depends_on_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_goal_dependencies_depends_on_id"), table_name="goal_dependencies"
    )
