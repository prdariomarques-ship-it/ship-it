...
async def store_memory(payload: MemoryCreate, db: DbSession, _: CurrentUser) -> MemoryRead:
    try:
        record = await memory_service.store(
            db, content=payload.content, source=payload.source, contact_id=payload.contact_id
        )
...
async def search_memory(
    ...
    try:
        results = await memory_service.search(query=q, limit=limit, contact_id=contact_id)
